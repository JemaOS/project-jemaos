#!/bin/bash
#
. $(dirname ${BASH_SOURCE[0]})/jemaos_version.sh

CHROMEOS_ARC_ANDROID_SDK_VERSION=28
CHROMEOS_ARC_VERSION=10061641
CHROMEOS_VERSION_AUSERVER=https://up.jemaos.io/service/update2
CHROMEOS_VERSION_DEVSERVER=https://devserver.jemaos.io:9999
...
if [[ -n "${CHROMEOS_BUILD}" ]] && [[ -n "${IMAGES_TO_BUILD}" ]] ; then
  ...
  export JEMAOS_RELEASE=$(get_jemaos_release_version)
fi

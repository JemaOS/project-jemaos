...
readonly BASE_VERSION_URL="https://devserver.jemaos.io/stateful-update"
readonly LOG_FILE="/var/log/crostini-jemaos-update.log"

readonly CURL_BIN="/usr/bin/curl"
readonly CURL_PARAMS="--silent --show-error --retry 2 --connect-timeout 7 --max-time 18"
readonly CURL="$CURL_BIN $CURL_PARAMS"

readonly LOCAL_JEMAOS_BUILD_TYPE="CHROMEOS_RELEASE_BUILD_TYPE"
readonly LOCAL_RELEASE_VERSION="CHROMEOS_RELEASE_VERSION"
readonly LSB_RELEASE_BOARD="CHROMEOS_RELEASE_BOARD"

readonly LOCAL_TATL_LSB_RELEASE="/mnt/stateful_partition/dev_image/tatl-jemaos/lsb-release"
readonly LOCAL_TAEL_LSB_RELEASE="/mnt/stateful_partition/dev_image/tael-jemaos/lsb-release"
readonly LOCAL_JEMAOS_LSB_RELEASE="/etc/lsb-release"
# readonly LOCAL_JEMAOS_LSB_RELEASE="./os-lsb-release"
...
readonly STATEFUL_UPDATE_SCRIPT="/usr/bin/stateful_update_jemaos"
...
read_os_boardname() {
  local name=""
  name=$(grep "$LSB_RELEASE_BOARD" "$LOCAL_JEMAOS_LSB_RELEASE" | awk -F '=' '{print $2}')
  OS_BOARDNAME="$name"
}
...
read_local_major_version() {
  grep "$LOCAL_JEMAOS_BUILD_TYPE" "$LOCAL_CROSTINI_LSB_RELEASE" 2>/dev/null | awk -F '=' '{print $2}' | awk '{print $3}' | tr -d 'v-'
}

read_os_major_version() {
  grep "$LOCAL_JEMAOS_BUILD_TYPE" "$LOCAL_JEMAOS_LSB_RELEASE" | awk -F '=' '{print $2}' | awk '{print $3}' | tr -d 'v' | cut -d '-' -f1
}
...
read_local_version() {
  ...
  if [[ ! -f "$LOCAL_JEMAOS_LSB_RELEASE" ]] || [[ ! -r "$LOCAL_JEMAOS_LSB_RELEASE" ]]; then
    logger "can't read file $LOCAL_JEMAOS_LSB_RELEASE"
    clean_exit
  fi
...
fetch_version_info() {
  ...
  local os_major_version="${OS_MAJOR_VERSION%.*}"
  local version_url="$BASE_VERSION_URL/${OS_BOARDNAME}/${os_major_version}_64bit/version"
...
upgrade() {
  export JEMAOS_STATEFUL_UPDATE_URL="$REMOTE_FILE_URL"
  export JEMAOS_STATEFUL_UPDATE_VERSION="$REMOTE_DETAIL_VERSION"
